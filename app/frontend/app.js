...
const refineTickersInput = document.getElementById("refineTickersInput");
const refineRiskInput = document.getElementById("refineRiskInput");
const llmStatus = document.getElementById("llmStatus");
const debugStatus = document.getElementById("debugStatus");
const welcomeOptions = document.getElementById("welcomeOptions");
const intakeForm = document.getElementById("intakeForm");
const riskInput = document.getElementById("riskInput");
const planningForInput = document.getElementById("planningForInput");
const birthYear1Input = document.getElementById("birthYear1Input");
const birthMonth1Input = document.getElementById("birthMonth1Input");
const birthYear2Input = document.getElementById("birthYear2Input");
const birthMonth2Input = document.getElementById("birthMonth2Input");
const birthDate2Label = document.getElementById("birthDate2Label");
const retirementInput = document.getElementById("retirementInput");
const spendingInput = document.getElementById("spendingInput");
const investmentValueInput = document.getElementById("investmentValueInput");
...
const retirementDateInput = document.getElementById("retirementDateInput");
const otherNotesInput = document.getElementById("otherNotesInput");
const analyzeForm = document.getElementById("analyzeForm");
const analyzeJson = document.getElementById("analyzeJson");
const analyzeCsv = document.getElementById("analyzeCsv");
const chatInput = document.getElementById("chatInput");
const authForm = document.getElementById("authForm");
const authEmailInput = document.getElementById("authEmailInput");
const authPasswordInput = document.getElementById("authPasswordInput");

const FORM_STATE_KEY = "portfolio-optimizer-form-state";
const USER_ID_KEY = "portfolio-optimizer-user-id";
...
function parseHorizonYears(text) {
  if (!text || typeof text !== "string") return null;
  const t = text.trim();
  // "10 years" or "10 yr"
  const m = t.match(/(\d+)\s*years?/i);
  if (m) return parseInt(m[1], 10);
  // "2038-06-30" or "2038" - years from now to retirement
...
const wordYears = { a: 1, one: 1, two: 2, three: 3, four: 4, five: 5, six: 6, seven: 7, eight: 8, nine: 9, ten: 10 };
function parseExpenses(spendingText) {
  const expenses = [];
  ...
  while ((m = rePurposeYears.exec(spendingText)) !== null) {
    ...
    if (amount > 0 && years >= 0) expenses.push({ years, amount });
  }
  ...
  while ((m = rePurposeWordYear.exec(spendingText)) !== null) {
    const amount = parseAmount(m[1]);
    const years = wordYears[m[3].toLowerCase()];
    if (amount > 0 && years != null) expenses.push({ years, amount });
  }
  const reYears = /([\d,.]+(?:\s*[KMB])?)\s+in\s+(\d+)\s*years?/gi;
  while ((m = reYears.exec(spendingText)) !== null) {
    const amount = parseAmount(m[1]);
    const years = parseInt(m[2], 10);
    if (amount > 0 && years >= 0) expenses.push({ years, amount });
  }
  const reWordYear = /([\d,.]+(?:\s*[KMB])?)\s+in\s+(a|one|two|three|four|five|six|seven|eight|nine|ten)\s*years?/gi;
  while ((m = reWordYear.exec(spendingText)) !== null) {
    const amount = parseAmount(m[1]);
    const years = wordYears[m[2].toLowerCase()];
    if (amount > 0 && years != null) expenses.push({ years, amount });
  }
  const reYear = /([\d,.]+(?:\s*[KMB])?)\s+in\s+(\d{4})\b/gi;
  while ((m = reYear.exec(spendingText)) !== null) {
    const amount = parseAmount(m[1]);
    const year = parseInt(m[2], 10);
    if (amount > 0 && year >= 2020 && year <= 2100) expenses.push({ years: year, amount });
  }
  return expenses;
}

const SESSION_KEY = "portfolio-optimizer-session";
localStorage.removeItem(SESSION_KEY);
const sessionId = crypto.randomUUID();

/** After Emu retirement backtest, user chose "Keep refining" — next money-manager call routes to Panda. */
let retirementRefineAfterEmu = false;

let currentIntent = "intake";
let savedIntakeMessage = "";
let userFilledIntakeForm = false;
let lastPortfolioComposition = null;
let lastPortfolioSectors = null;
let lastPortfolioIndustries = null;
...
function addMessage(role, text) {
  // Strip machine-readable JSON block and spurious empty ```json  ``` blocks
  if (text && typeof text === "string") {
    text = text.replace(/<<<PORTFOLIOS_JSON>>>[\s\S]*?<<<END_PORTFOLIOS_JSON>>>/g, "").replace(/```\s*json\s*```/gi, "").trim();
  }
  const row = document.createElement("div");
  row.className = `chat-row ${role}`;
  const marker = "\n\n[LLM_RAW]\n";
  if (text && text.includes(marker)) {
    const [main, raw] = text.split(marker);
    const mainBlock = document.createElement("div");
    mainBlock.textContent = main;
    row.appendChild(mainBlock);
    const rawBlock = document.createElement("pre");
    rawBlock.className = "llm-raw";
    rawBlock.textContent = raw || "";
    row.appendChild(rawBlock);
  } else {
...
function removeChoiceButtons() {
  const existing = chatLog.querySelectorAll(".choice-buttons");
  existing.forEach((el) => el.remove());
}

function addChoiceButtons(choices, onPick) {
  removeChoiceButtons();
...
function showAuthForm() {
  ...
  if (refineForm) refineForm.classList.add("hidden");
  if (authForm) {
...
async function savePortfolio(userId) {
  if (!lastPortfolioComposition || typeof lastPortfolioComposition !== "object") {
    addMessage("error", "No portfolio to save.");
    return;
  }
  try {
    await postJson("/api/portfolio/save", {
      user_id: userId || localStorage.getItem(USER_ID_KEY) || undefined,
      session_id: sessionId,
      portfolio_ticker_weights: lastPortfolioComposition,
      portfolio_sector_weights: lastPortfolioSectors || undefined,
      portfolio_industry_weights: lastPortfolioIndustries || undefined,
    });
    await postJson("/api/portfolio", {
      session_id: sessionId,
      holdings: lastPortfolioComposition,
    });
    addMessage("assistant", "Portfolio saved to database. You can run backtests or rebalancing from the chat.");
  } catch (err) {
    ...
    if (action.type === "show_portfolio_choices") {
      addChoiceButtons(
        [
          ...
          { label: "Keep Refining", value: "I want to keep refining" },
        ],
        sendQuickMessage,
      );
      ...
      addChoiceButtons(
        ...
        (value) => {
          if (value === "__save_portfolio__") {
            ...
            if (storedUserId) {
              savePortfolio(storedUserId);
            } else {
              showAuthForm();
            }
          } else if (value === "__keep_refining__") {
            retirementRefineAfterEmu = refineAdvisor === "Panda";
            if (refineAdvisor === "Panda") {
              addMessage(
                "assistant",
                "Emu: I'm handing you back to Panda so we can revise all three retirement portfolios—conservative, moderate, and aggressive—based on what you type next.",
              );
            }
            if (chatInput) chatInput.classList.add("hidden");
            if (refineForm) {
              refineForm.classList.remove("hidden");
              if (refineTickersInput) refineTickersInput.focus();
            }
          }
        },
      );
...
function renderInlineCharts(artifacts, parentEl) {
  ...
  if (scenarios && scenarios.length) {
    ...
    const isRetirementArtifact = artifacts.is_retirement === true;
    ...
    const corrCard = assetCorr && assetCorr.rows && assetCorr.rows.length
      ? (() => {
          ...
          _renderAssetCorrelationTable(c.querySelector(".chart-div"), assetCorr, isRetirementArtifact);
          return c;
        })()
      : null;
...
function _buildTimelineData(scenarios, artifacts) {
  ...
  return {
    ...
    events: parseExpenses(String(intake.spending || '')).map((e) => ({
      year: e.years,
...
async function postJson(url, payload) {
  ...
  if (!res.ok) {
    ...
    const msg = typeof detail.detail === "string" ? detail.detail
      : Array.isArray(detail.detail) ? detail.detail.map((d) => d.msg || d).join("; ")
      : "Request failed";
    throw new Error(msg);
  }
  return res.json();
...
function handleArtifacts(data) {
  ...
  if (a.meta) {
    updateLlmStatus(a.meta);
    updateDebugStatus(a.meta);
  }
  if (a.portfolio_composition) {
    lastPortfolioComposition = a.portfolio_composition;
  }
  if (a.portfolio_sectors) lastPortfolioSectors = a.portfolio_sectors;
  if (a.portfolio_industries) lastPortfolioIndustries = a.portfolio_industries;
  if (a.portfolio_composition || a.scenarios || (a.all_portfolios && Object.keys(a.all_portfolios).length)) {
    renderInlineCharts(a);
  }
}

// ---- Quick message (choice buttons) ----

function attachRetirementRefineAfterEmuPayload(payload) {
  if (!retirementRefineAfterEmu) return;
  retirementRefineAfterEmu = false;
...
async function sendQuickMessage(text) {
  ...
  try {
    ...
    attachRetirementRefineAfterEmuPayload(payload);
...
function getIntakeFromForm() {
  ...
  const planningFor = planningForInput?.value || "self";
  const birthYear1 = parseInt(birthYear1Input?.value, 10);
  const birthMonth1 = parseInt(birthMonth1Input?.value, 10);
  const birthYear2 = parseInt(birthYear2Input?.value, 10);
  const birthMonth2 = parseInt(birthMonth2Input?.value, 10);
  const retirement = retirementInput?.value?.trim() || "";
  const retirementDate = retirementDateInput?.value?.trim() || "";
  const investmentValue = investmentValueInput?.value?.trim() || "";
  const monthlyContribution = monthlyContributionInput?.value?.trim() || "";
  const monthlyExpense = monthlyExpenseInput?.value?.trim() || "";
  const spending = spendingInput?.value?.trim() || "";
  const initialVal = parseAmount(investmentValue) || 1000;
  const monthlySav = parseAmount(monthlyContribution) || 0;
  const horizonYr = parseHorizonYears(retirement) || parseHorizonYears(retirementDate);
  const monthlyExpenseVal = parseAmount(monthlyExpense) || 0;
  const birthDates = [];
  if (birthYear1 && birthYear1 >= 1920 && birthYear1 <= 2010) {
    birthDates.push({ year: birthYear1, month: (birthMonth1 >= 1 && birthMonth1 <= 12) ? birthMonth1 : 6 });
  }
  if (planningFor === "couple" && birthYear2 && birthYear2 >= 1920 && birthYear2 <= 2010) {
  ...
  return {
    ...
    horizon_years: horizonYr || undefined,
...
async function submitChatMessage() {
  ...
  try {
    const endpoint =
      currentIntent === "money_manager" ? "/api/chat/money-manager" : "/api/chat/intake";
    const payload = { session_id: sessionId, message };
    if (endpoint === "/api/chat/money-manager") {
      const intake = getIntakeFromForm();
...
function saveFormState() {
  const state = {
    planningFor: planningForInput?.value || "self",
    birthYear1: birthYear1Input?.value || "",
    birthMonth1: birthMonth1Input?.value || "",
    birthYear2: birthYear2Input?.value || "",
    birthMonth2: birthMonth2Input?.value || "",
    risk: riskInput?.value || "",
    retirement: retirementInput?.value || "",
    spending: spendingInput?.value || "",
    investmentValue: investmentValueInput?.value || "",
    monthlyContribution: monthlyContributionInput?.value || "",
    monthlyExpense: monthlyExpenseInput?.value || "",
    retirementDate: retirementDateInput?.value || "",
    otherNotes: otherNotesInput?.value || "",
  };
...
function loadFormState() {
  ...
  try {
    ...
    if (planningForInput) planningForInput.value = state.planningFor || "self";
    if (birthYear1Input) birthYear1Input.value = state.birthYear1 || "";
    if (birthMonth1Input) birthMonth1Input.value = state.birthMonth1 || "";
    if (birthYear2Input) birthYear2Input.value = state.birthYear2 || "";
    if (birthMonth2Input) birthMonth2Input.value = state.birthMonth2 || "";
    if (birthDate2Label) birthDate2Label.classList.toggle("hidden", state.planningFor !== "couple");
    if (riskInput) riskInput.value = state.risk || "";
    if (retirementInput) retirementInput.value = state.retirement || "";
    if (spendingInput) spendingInput.value = state.spending || "";
    if (investmentValueInput) investmentValueInput.value = state.investmentValue || "";
    if (monthlyContributionInput)
      monthlyContributionInput.value = state.monthlyContribution || "";
    if (monthlyExpenseInput) monthlyExpenseInput.value = state.monthlyExpense || "";
    if (retirementDateInput) retirementDateInput.value = state.retirementDate || "";
    if (otherNotesInput) otherNotesInput.value = state.otherNotes || "";
    if (state.investmentValue || state.retirement || state.retirementDate || state.monthlyContribution) {
      userFilledIntakeForm = true;
    }
  } catch {
...
function showWelcome() {
  addMessage(
    "assistant",
    "Hello, I'm Quala, your AI money manager. Please fill in a few details so I can understand your goals."
  );
  if (intakeForm) intakeForm.classList.remove("hidden");
  if (welcomeOptions) welcomeOptions.classList.add("hidden");
  if (chatInput) chatInput.classList.add("hidden");
  if (analyzeForm) analyzeForm.classList.add("hidden");
}

async function sendIntentSelection(intent) {
  ...
  await postJson("/api/session/intent", {
    session_id: sessionId,
    intent,
  });
  ...
  if (intent === "intake") {
    ...
    try {
      const data = await postJson("/api/chat/money-manager", {
        session_id: sessionId,
        message: savedIntakeMessage,
      });
      addMessage("assistant", data.reply);
      handleArtifacts(data);
      handleActions(data.actions);
  ...
  if (intent === "backtest") {
    ...
    try {
      ...
      handleArtifacts(data);
      const hasScenarios =
        data.artifacts && Array.isArray(data.artifacts.scenarios) && data.artifacts.scenarios.length;
      if (hasScenarios) {
        if (analyzeForm) analyzeForm.classList.add("hidden");
        if (chatInput) chatInput.classList.remove("hidden");
      }
...
function updateLlmStatus(meta) {
  if (!llmStatus || !meta) return;
  const debugEnabled = new URLSearchParams(window.location.search).get("debug") === "1";
  if (!debugEnabled) {
    llmStatus.classList.add("hidden");
    return;
  }
  const backend = meta.llm_backend || "unknown";
  const model = meta.llm_model || "unknown";
  const usedForReply = Boolean(meta.llm_used_for_reply);
  const usedAny = Boolean(meta.llm_used);
  const error = meta.llm_error;
  if (usedForReply && !error) {
    llmStatus.textContent = `LLM: ${backend} (${model})`;
    llmStatus.classList.remove("hidden");
    llmStatus.classList.add("ok");
    return;
  }
  if (usedAny && !error) {
    llmStatus.textContent = `LLM available (${backend}/${model}). Reply was deterministic.`;
    llmStatus.classList.remove("hidden");
    llmStatus.classList.add("ok");
    return;
  }
  const reason = error ? `Error: ${error}` : "LLM not used.";
  llmStatus.textContent = `LLM unavailable (${backend}/${model}). ${reason}`;
  llmStatus.classList.remove("hidden");
  llmStatus.classList.remove("ok");
}

function updateDebugStatus(meta) {
  if (!debugStatus || !meta) return;
  const enabled = new URLSearchParams(window.location.search).get("debug") === "1";
  if (!enabled) {
    debugStatus.classList.add("hidden");
    return;
  }
  const intent = meta.intent_candidate || meta.intent || "unknown";
  const confidence = meta.intent_confidence ?? "n/a";
  const source = meta.intent_source || "unknown";
  const backend = meta.llm_backend || "unknown";
  const model = meta.llm_model || "unknown";
  debugStatus.textContent = `Intent: ${intent} | Confidence: ${confidence} | Source: ${source} | LLM: ${backend} (${model})`;
...
if (welcomeOptions) {
  welcomeOptions.addEventListener("click", (event) => {
    const button = event.target.closest("button");
    if (!button) return;
    const intent = button.getAttribute("data-intent");
    if (intent) {
      sendIntentSelection(intent).catch((err) => addMessage("error", err.message));
    }
  });
}
...
if (intakeForm) {
  intakeForm.addEventListener("submit", async (event) => {
    ...
    const birthYear2 = parseInt(birthYear2Input?.value, 10);
    const birthMonth2 = parseInt(birthMonth2Input?.value, 10);
    const risk = riskInput?.value.trim() || "medium risk tolerance";
    const retirement = retirementInput?.value.trim() || "10 years";
    const spending = spendingInput?.value.trim() || "no big spending expected";
    const investmentValue = investmentValueInput?.value.trim() || "1000";
    const monthlyContribution =
      monthlyContributionInput?.value.trim() || "no monthly contributions";
    const retirementDate = retirementDateInput?.value.trim() || "not specified";
    const otherNotes = otherNotesInput?.value.trim() || "none";
    const monthlyExpense = monthlyExpenseInput?.value.trim() || "not specified";
    savedIntakeMessage = [
      ...
      `Exact retirement date: ${retirementDate}.`,
      `Other notes: ${otherNotes}.`,
    ].join("\n");
    addMessage("user", savedIntakeMessage);
    saveFormState();
    userFilledIntakeForm = true;
    ...
    const monthlySav = parseAmount(monthlyContribution) || 0;
    const horizonYr = parseHorizonYears(retirement) || parseHorizonYears(retirementDate);
    const monthlyExpenseVal = parseAmount(monthlyExpenseInput?.value) || 0;
    const birthDates = [];
    if (birthYear1 && birthYear1 >= 1920 && birthYear1 <= 2010) {
      birthDates.push({ year: birthYear1, month: (birthMonth1 >= 1 && birthMonth1 <= 12) ? birthMonth1 : 6 });
    }
    ...
    const displayUnit = parseDisplayUnit(investmentValue) || null;
    // Always sync intake to backend so backtest uses correct initial_value
    try {
    ...
    if (welcomeOptions) welcomeOptions.classList.remove("hidden");
  });
}

if (planningForInput) {
  planningForInput.addEventListener("change", () => {
    if (birthDate2Label) birthDate2Label.classList.toggle("hidden", planningForInput.value !== "couple");
    saveFormState();
  });
}
if (birthYear1Input) birthYear1Input.addEventListener("input", saveFormState);
if (birthMonth1Input) birthMonth1Input.addEventListener("input", saveFormState);
if (birthYear2Input) birthYear2Input.addEventListener("input", saveFormState);
if (birthMonth2Input) birthMonth2Input.addEventListener("input", saveFormState);
if (riskInput) riskInput.addEventListener("input", saveFormState);
if (retirementInput) retirementInput.addEventListener("input", saveFormState);
if (spendingInput) spendingInput.addEventListener("input", saveFormState);
if (investmentValueInput) investmentValueInput.addEventListener("input", saveFormState);
if (monthlyContributionInput) monthlyContributionInput.addEventListener("input", saveFormState);
if (monthlyExpenseInput) monthlyExpenseInput.addEventListener("input", saveFormState);
if (retirementDateInput) retirementDateInput.addEventListener("input", saveFormState);
if (otherNotesInput) otherNotesInput.addEventListener("input", saveFormState);

if (refineForm) {
  refineForm.addEventListener("submit", async (event) => {
    event.preventDefault();
    const tickers = (refineTickersInput?.value || "").trim();
    const risk = (refineRiskInput?.value || "").trim();
    if (!tickers && !risk) {
      addMessage("error", "Please add at least one ticker or update your risk appetite.");
      return;
    }
    const parts = [];
    if (tickers) parts.push(`Add these tickers to my portfolio: ${tickers}`);
    if (risk) parts.push(`My risk appetite: ${risk}`);
    const message = `I want to refine the portfolio. ${parts.join(". ")}`;
    ...
    try {
      ...
      handleArtifacts(data);
      handleActions(data.actions);
      if (refineTickersInput) refineTickersInput.value = "";
      if (refineRiskInput) refineRiskInput.value = "";
    } catch (err) {
      addMessage("error", err.message);
    }
  });
...
if (authForm) {
  authForm.addEventListener("submit", async (event) => {
    ...
    try {
      let userId;
      try {
        const res = await postJson("/api/auth/register", { email_id: email, password });
        userId = res.user_id;
      } catch (regErr) {
        if (regErr.message && regErr.message.toLowerCase().includes("already")) {
          const loginRes = await postJson("/api/auth/login", { email_id: email, password });
...
if (analyzeForm) {
  analyzeForm.addEventListener("submit", async (event) => {
    event.preventDefault();
    const jsonText = analyzeJson?.value.trim();
    const looseFormatMessage =
      "Provide JSON like {\"AAPL\": 0.5, \"MSFT\": 0.5} or use the CSV upload.";
    if (!jsonText && (!analyzeCsv || !analyzeCsv.files.length)) {
      addMessage("error", "Provide JSON weights or upload a CSV.");
      return;
    }
    ...
    try {
      if (jsonText) {
        let holdings;
        try {
          holdings = JSON.parse(jsonText);
        } catch (err) {
          const res = await postJson("/api/chat/intake", {
            session_id: sessionId,
            message: jsonText,
          });
          if (res?.artifacts?.portfolio && Object.keys(res.artifacts.portfolio).length) {
            holdings = res.artifacts.portfolio;
          } else {
            throw new Error(looseFormatMessage);
          }
        }
        await postJson("/api/portfolio", {
          session_id: sessionId,
      ...
      } else if (analyzeCsv && analyzeCsv.files.length) {
        const form = new FormData();
        form.append("session_id", sessionId);
        form.append("file", analyzeCsv.files[0]);
